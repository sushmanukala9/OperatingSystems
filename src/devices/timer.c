...
timer_interrupt(struct intr_frame *args UNUSED)
{
  ...
  spinlock_acquire(&get_cpu()->lock_sleep_list);
  int64_t cur_ticks = timer_ticks();
  struct list_elem *e = list_begin(&get_cpu()->sleeping_thread_list);

  while (e != list_end(&get_cpu()->sleeping_thread_list))
  {
  ...
  spinlock_release(&get_cpu()->lock_sleep_list);
}
