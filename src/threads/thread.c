...
void thread_init(void)
{
  ...
  list_init(&bcpu->sleeping_thread_list);

  sched_init(&bcpu->rq);
  spinlock_init(&bcpu->rq.lock);
  /* Set up a thread structure for the running thread. */
  initial_thread = running_thread();
  // bcpu->rq.sum_of_weights += initial_thread->weight;
...
void thread_init_on_ap(void)
{
  ...
  list_init(&cpu->sleeping_thread_list);
...
void load_balance(void)
{
  ...
  struct cpu *busiest_cpu = NULL;
  int64_t busiest_cpu_load = 0;
  int64_t current_cpu_load = current_cpu->rq.sum_of_weights;
  int64_t imbalance;

  // Identify the busiest CPU
  for (int i = 0; i < ncpu; ++i)
  {
    if (cpus[i].rq.sum_of_weights > busiest_cpu_load && &cpus[i] != current_cpu)
    {
      busiest_cpu_load = cpus[i].rq.sum_of_weights;
      busiest_cpu = &cpus[i];
    }
  }
  ...
  if (!busiest_cpu)
  {
    return; // No busier CPU found
  }

  imbalance = busiest_cpu_load - current_cpu_load;

  // Check if it's worth balancing
  if (imbalance * 4 >= busiest_cpu_load)
  {
    spinlock_acquire(&busiest_cpu->rq.lock);

    // Verify again inside lock to prevent race conditions
    if (!list_empty(&busiest_cpu->rq.ready_list))
    {
      struct list_elem *e = list_pop_back(&busiest_cpu->rq.ready_list);
      struct thread *t = list_entry(e, struct thread, elem);

      // Adjust vruntime for the migrated thread
      int64_t vruntime_diff = busiest_cpu->rq.min_vruntime - current_cpu->rq.min_vruntime;
      t->vruntime += vruntime_diff;
      t->vruntime_0 = t->vruntime;

      // Adjusting the busy CPU's weights and thread count
      busiest_cpu->rq.sum_of_weights -= prio_to_weight[t->nice - NICE_MIN];
      busiest_cpu->rq.nr_ready--;

      spinlock_release(&busiest_cpu->rq.lock);

      // Add thread to current CPU's ready list
      spinlock_acquire(&current_cpu->rq.lock);
      t->cpu = current_cpu;
      list_push_back(&current_cpu->rq.ready_list, &t->elem);
      current_cpu->rq.sum_of_weights += prio_to_weight[t->nice - NICE_MIN];
      current_cpu->rq.nr_ready++;

      spinlock_release(&current_cpu->rq.lock);

      // Since a new thread is now available, we should yield
      thread_yield();
    }
    else
    {
...
static void idle(void *idle_started_ UNUSED)
{
  for (;;)
  {
    ...
    load_balance(); // Attempt to balance load when idle

    // Recheck if still idle after load balancing attempt
    if (!list_empty(&get_cpu()->rq.ready_list))
    {
      intr_enable();
      thread_yield();
      continue;
...
bool time_to_wakeup(const struct list_elem *element1, const struct list_elem *element2)
{
  const struct thread *first_thread = list_entry(element1, struct thread, allelem);
  const struct thread *second_thread = list_entry(element2, struct thread, allelem);
  return first_thread->wake_time < second_thread->wake_time;
}
...
void thread_sleep(int64_t ticks)
{
  ...
  spinlock_acquire(&cpu->lock_sleep_list);

  struct thread *current_thread = thread_current();
  current_thread->wake_time = start + ticks;

  list_insert_ordered(&cpu->sleeping_thread_list, &current_thread->elem, time_to_wakeup, NULL);

  thread_block(&cpu->lock_sleep_list);

  spinlock_release(&cpu->lock_sleep_list);
  intr_enable();
}
